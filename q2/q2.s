...
input_loop:
    ...
    mv a1, t0          # save loop counter in a1 before call (or use stack)
    call atoi          # converts a0 to int value as it reads it as string
    mv t0, a1          # restore loop counter after call
...
while:
    ...
    blt s5, x0, push   # initally s5 is -1 and when stack is empty it goes to push
    ...
    slli t1, t4, 3             # here we are loading the value of that index form array into t5
    add t1, s2, t1
    ld t5, 0(t1)               

    bgt t5, t3, after_while        # arr[stack[top]] > arr[i] we continue to while-done
    addi s5, s5, -1               # s5 is basically maintaining count of the number of elements-1 in stack
    j while
    
after_while:
    slli t1, t0, 3               # t1 = offest which is i*8
